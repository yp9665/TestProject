...
var ControllerFinishRequest = /** @class */ (function () {
    ...
    ControllerFinishRequest.prototype.run = function () {
        ...
        (0, request_1.default)({
            ...
            onError: function (error) {
                _this.controller.log("Finalize upload failed. ".concat(error.message));
    ...
    ControllerFinishRequest.prototype.onSuccessFinishUpload = function (response) {
        ...
        if (this.controller.uploadStatus.state !== types_1.FileUploadServiceState.Verifying) {
            this.controller.log("Verifying: Upload status has changed, current status:  ".concat(this.controller.uploadStatus.state));
        }
        ...
        if (response.error === false && response.data.state === 'Done') {
            ...
            if (response.data.missing_chunks && response.data.missing_chunks.length > 0) {
                ...
                this.controller.message("Finalizing found missing  ".concat(response.data.missing_chunks.length, " chunks. Requeuing chunks."), types_1.MessageLevel.Information);
                (0, telemetry_1.trackEvent)('UploadAutoRecovery', {
    ...
    ControllerFinishRequest.prototype.completeUpload = function (uploadResponse) {
        ...
        this.controller.log("".concat(uploadResponse.data.message, ": ").concat(uploadResponse.data.location));
        (0, telemetry_1.trackEvent)('UploadFinalized', { AssetId: this.controller.uploadData.assetId, CorrelationId: this.controller.uploadData.correlationId });
        var averageSpeed = (0, metrics_1.calculateAverageSpeedWithChunks)(this.controller.uploadStatus.startTime, this.controller.uploadStatus.blocksCompleted, this.controller.uploadData.chunkSize);
        ...
        var completeMessage = "Upload completed in:  ".concat(uploadStats.totalTimeInSeconds, " seconds. Average speed: ").concat(uploadStats.averageSpeedInMbps, " Mbps.");
        (0, telemetry_1.trackEvent)('UploadCompleted', {
