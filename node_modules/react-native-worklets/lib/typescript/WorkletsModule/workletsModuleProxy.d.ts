...
export interface WorkletsModuleProxy {
    ...
    makeShareableImport<TValue>(source: string, imported: string): ShareableRef<TValue>;
    ...
    makeShareableTurboModuleLike<TProps extends object, TProto extends object>(props: TProps, proto: TProto): ShareableRef<TProps>;
    makeShareableObject<T extends object>(obj: T, shouldRetainRemote: boolean, nativeStateSource?: object): ShareableRef<T>;
    makeShareableHostObject<T extends object>(obj: T): ShareableRef<T>;
    makeShareableArray(array: unknown[], shouldRetainRemote: boolean): ShareableRef<unknown[]>;
    makeShareableMap<TKey, TValue>(keys: TKey[], values: TValue[]): ShareableRef<Map<TKey, TValue>>;
    makeShareableSet<TValues>(values: TValues[]): ShareableRef<Set<TValues>>;
    makeShareableInitializer(obj: object): ShareableRef<object>;
    makeShareableFunction<TArgs extends unknown[], TReturn>(func: (...args: TArgs) => TReturn): ShareableRef<TReturn>;
    makeShareableWorklet(worklet: object, shouldPersistRemote: boolean): ShareableRef<object>;
